...
class UNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        up4= self.cuda().up4(up_, down3)
        up3= self.cuda().up3(up4, down2)
        up2= self.cuda().up2(up3, down1)
        up1= self.cuda().up1(up2, in64)
...
class unetConv2(nn.Module):
    ...
    def forward(self, inputs):
        ...
        outputs= self.cuda().conv1(inputs)
        outputs= self.cuda().conv2(outputs)
        # -shaharnadav_wp-UNET
        return outputs
